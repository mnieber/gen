...
from moonleap.builder.add_resources_to_blocks import (
    add_meta_data_to_blocks,
    create_resource_and_add_to_block,
)
...
def _to_list_of_relations(x, rule):
...
def create_resources(blocks):
    ...
    rule_tuples = []
    for block in blocks:
        _process_relations(get_relations(block), block, rule_tuples)
    ...
    while rule_tuples:
        ...
        if result:
            _process_relations(_to_list_of_relations(result, rule), block, rule_tuples)
