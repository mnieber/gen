...
def django_uses_postgres_service(django_app, postgres_service):
    add(django_app, PkgDependency(["postgresql-client"], target="dev"))
    add(django_app, PipRequirement(["psycopg2"], target="base"))
    add(django_app, PipDependency(["pgcli==3.4.1"], target="dev"))
